...
static void default_element_added(GstBin *bin, GstElement *element, gpointer user_data)
{
  ...
  {
      if (g_strrstr(GST_ELEMENT_NAME(element), "westeros") || \
          g_strrstr(GST_ELEMENT_NAME(element), "amlvideosink"))
      {
...
static void about_to_finish(gpointer handle)
{
  GstPlay *player = handle;
  if (NULL == handle)
  {
...
//  g_idle_add(deferred_stop, handle);

  callback_to_app(player, AGMP_MESSAGE_ABOUT_FINISH, player->userdata);
}
...
GstElement* agmp_get_vsink(AGMP_HANDLE handle)
{
...
int agmp_unset_uri(AGMP_HANDLE handle)
{
  CHECK_POINTER_VALID(handle);
  GstPlay* player = (GstPlay*)handle;
  g_free (player->uri);
...
AGMP_HANDLE agmp_init (void)
{
  ...
  g_signal_connect_swapped(player->playbin, "about-to-finish", G_CALLBACK(about_to_finish), player);
...
int agmp_prepare (AGMP_HANDLE handle, char* msg)
{
  CHECK_POINTER_VALID(handle);
  GstPlay* player = (GstPlay*)handle;
  gboolean ret = TRUE;
  ...
  if (AGMP_STATUS_PREPARED == player->status)
  {
    ...
    if (msg != NULL) sprintf(msg, "already playing: %d", player->status);
    return AAMP_SUCCESS;
  }

  if (player->status != AGMP_STATUS_NULL && player->status != AGMP_STATUS_STOPPED)
  {
    GST_ERROR ("can't be called in this state: %d.", player->status);
    if (msg != NULL) sprintf(msg, "can't be called in this state: %d", player->status);
  ...
  case GST_STATE_CHANGE_FAILURE:
    ...
    if (msg != NULL) sprintf(msg, "Pipeline state change fail");
    ret = FALSE;
    break;
  ...
  if (msg != NULL) sprintf(msg, "prepare stream done");
  set_aamp_state(player, eSTATE_PREPARED);

  return AAMP_SUCCESS;
}

int agmp_play (AGMP_HANDLE handle, char* msg)
{
  CHECK_POINTER_VALID(handle);
  GstPlay* player = (GstPlay*)handle;
  ...
  if (player->status != AGMP_STATUS_PREPARED && player->status != AGMP_STATUS_PAUSED && player->status != AGMP_STATUS_STOPPED)
  {
    GST_ERROR ("can't be called in this state: %d.", player->status);
    if (msg != NULL) sprintf(msg, "can't be called in this state: %d.", player->status);
    return AAMP_FAILED_IN_THIS_STATE;
  ...
  {
      GstState state;
      ret = gst_element_get_state(player->playbin, &state, NULL, 3 * GST_SECOND);
      if (ret == GST_STATE_CHANGE_FAILURE)
      {
          GST_ERROR ("failed to change state to PLAYING");
          if (msg != NULL) sprintf(msg, "failed to change state to PLAYING");
          return AAMP_FAILED;
      }
  }

  GST_DEBUG ("playing stream done.");
  if (msg != NULL) sprintf(msg, "playing stream done");

  return AAMP_SUCCESS;
}

int agmp_pause (AGMP_HANDLE handle, char* msg)
{
  CHECK_POINTER_VALID(handle);
  GstPlay* player = (GstPlay*)handle;
  ...
  if (AGMP_STATUS_PAUSED == player->status)
  {
    ...
    if (msg != NULL) sprintf(msg, "already paused: %d", player->status);
    return AAMP_SUCCESS;
  }
  ...
  if (player->buffering) {
    ...
    if (msg != NULL) sprintf(msg, "buffering, no need pause");
    return AAMP_SUCCESS;
  }
  ...
  GST_DEBUG ("paused stream done.");
  if (msg != NULL) sprintf(msg, "paused stream done");

  return AAMP_SUCCESS;
}

int agmp_stop (AGMP_HANDLE handle, char* msg)
{
  ...
  if (AGMP_STATUS_STOPPED == player->status)
  {
    GST_ERROR ("already stoped: %d.", player->status);
    if (msg != NULL) sprintf(msg, "already stoped: %d", player->status);
    return AAMP_SUCCESS;
  }
  ...
  ret = gst_element_get_state(player->playbin, &cur, &pending, 3 * GST_SECOND);
  if (ret == GST_STATE_CHANGE_FAILURE)
  {
    GST_ERROR ("failed to change state to STOPPED");
    if (msg != NULL) sprintf(msg, "failed to change state to STOPPED");
    return AAMP_FAILED;
  }
  ...
  player->status = AGMP_STATUS_STOPPED;

  GST_DEBUG ("stopped stream done.");
  if (msg != NULL) sprintf(msg, "stopped stream done");
...
void agmp_deferred_callback(AGMP_HANDLE handle, int (*cb)(AGMP_HANDLE handle))
{
  if (NULL == handle)
  {
    GST_ERROR ("handle is null.");
    ...
    case GST_MESSAGE_STATE_CHANGED:
    {
      ...
      if (GST_MESSAGE_SRC (msg) == GST_OBJECT (player->playbin)) {
        ...
        if (state == GST_STATE_VOID_PENDING) {
          // printf("bus message status change to pending, %p", player->playbin);
          ...
          // printf("bus message status change to playing, %p", player->playbin);
