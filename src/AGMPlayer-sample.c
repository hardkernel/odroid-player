...
{
  //duration
  long long dur = agmp_get_duration(handle);
  printf ("%ld:%02ld:%02ld.%03ld", (long)dur/SECOND/3600, (long)dur/SECOND/60%60, (long)dur/SECOND%60, (long)dur%SECOND);
  //track number
  ...
  for (int i=0; i < n_video; i++) {
    ...
    printf ("video stream:%d, codec:%s, container:%s, width:%d, height:%d, framerate:%d\n", \
  ...
  for (int i=0; i < n_audio; i++) {
    ...
    printf ("audio stream:%d, codec:%s, container:%s, samples:%d, channels:%d, rate:%d\n", \
  ...
  for (int i=0; i < n_text; i++) {
    ...
    printf ("subtitle stream:%d, language:%s\n", text_info.track_id, text_info.lang);
  }
}
...
static void agmp_message_callback(AGMP_HANDLE handle, AGMP_MESSAGE_TYPE type, void* userdata)
{
  switch (type)
  {
    case AGMP_MESSAGE_BUFFERING:
      printf ("message BUFFERING: %d%%.....\r", agmp_get_buffering_percent(handle));
      break;
    case AGMP_MESSAGE_ASYNC_DONE:
      {
        printf ("message ASYNC_DONE.....\n");
    ...
    case AGMP_MESSAGE_VIDEO_UNDERFLOW:
      printf ("message AGMP_MESSAGE_VIDEO_UNDERFLOW.....\n");
      break;
    case AGMP_MESSAGE_AUDIO_UNDERFLOW:
      printf ("message AGMP_MESSAGE_AUDIO_UNDERFLOW.....\n");
      break;
    case AGMP_MESSAGE_FIRST_VFRAME:
      printf ("message AGMP_MESSAGE_FIRST_VFRAME.....\n");
      break;
    case AGMP_MESSAGE_FIRST_AFRAME:
      printf ("message AGMP_MESSAGE_FIRST_AFRAME.....\n");
      break;
    case AGMP_MESSAGE_MEDIA_INFO_CHANGED:
      printf ("message AGMP_MESSAGE_MEDIA_INFO_CHANGED.....\n");
      break;
    default:
      break;
  }
...
static bool play_prev (AGMP_HANDLE handle)
{
  ...
  {
    printf ("reached the beginning of the playlist!!!\n");
    return FALSE;
  }
  agmp_stop (handle);
  agmp_set_uri(handle, file_list.uris[file_list.cur_index]);
...
int main (int argc, char **argv)
{
  ...
  if (argc < 2)
  {
    printf ("Usage: %s FILE1|URI1 [FILE2|URI2] [FILE3|URI3] ...", "AGMPlayer-sample");
    printf ("\n\n");
  ...
  for (int i = 1; i < argc; i++) {
    if (strncmp ( argv[i], "--", 2 )) {
      if (file_list.filenum >= FILE_NUM_MAX) {
        printf ("[AGMPlayer-sample]the number of filename is out of range\n");
        continue;
      }
      file_list.uris[file_list.filenum++] = argv[i];
      printf ("[AGMPlayer-sample]got uri: %s\n", argv[i]);
    }
    else {
      if (sscanf(argv[i], "--license-url=%s", &license_url) >= 1)
      {
        printf ("[AGMPlayer-sample]got license-url: %s\n", license_url);
      }
      else if(sscanf(argv[i], "--window-size=%d,%d,%d,%d", &x, &y, &w, &h) >= 1)
      {
        printf ("[AGMPlayer-sample]got window-size=%d,%d,%d,%d\n", x, y, w, h);
      }
      else {
        printf ("[AGMPlayer-sample]unknown param: %s\n", argv[i]);
        return 0;
      }
    }
  }

  printf ("[AGMPlayer-sample] found source: %d\n", file_list.filenum);
  if (NULL == file_list.uris[0])
  {
    printf ("Usage: %s FILE1|URI1 [FILE2|URI2] [FILE3|URI3] ...\n", "AGMPlayer-sample");
  ...
  if (handle == NULL) {
    printf("[AGMPlayer-sample]Failed to create 'playbin' element. Check your GStreamer installation.\n");
    return EXIT_FAILURE;
  }
  ...
  printf("[AGMPlayer-sample]I am main function, waitting for quit.\n");

  pthread_join(cmdThreadId, NULL);
  /* clean up */
  agmp_exit(handle);

  printf("[AGMPlayer-sample]main function quit\n");
  return 0;
}
